...
import com.github.lly835.bean.China;
...
//            JSONObject jsonData = JSONObject.parseObject(datas);
    ...
    public static String getTodayWeatherChina(String keyWord)//Map<String, Object>
            throws Exception {
        // 连接中央气象台的API
...
//          datas = datas.replaceAll("\\@","");///
            ...
            String cities;//=datas.replaceAll("@","");///
            //JSONObject json = (JSONObject) xs.read(cities);//datas
            JSONObject json = (JSONObject) xs.read(datas);//
            logger.info("********** 1 responseJson="+json.toString()+" **********");///@怎么去掉
            cities=json.toString().replaceAll("@","");
            json= JSONObject.fromObject(cities);
            logger.info("********** 2 responseJson="+json.getJSONArray("city").toString()+" **********");///待优化,ok
            JSONArray citiesJson=json.getJSONArray("city");
            for (int i = 0; i < citiesJson.size(); i++) {
                JSONObject city =citiesJson.getJSONObject(i);
                System.out.println(city.getString("quName")+","+city.getString("cityname")
                        +":"+city.getString("stateDetailed")+","+city.getString("tem1")
                        +"~"+city.getString("tem2")+","+city.getString("windState"));
                if(city.getString("quName").equals(keyWord)||city.getString("cityname").equals(keyWord)){
//                    map.put("quName", city.getString("quName"));// 城市
//                    map.put("cityname", city.getString("cityname"));// 最高温度
//                    map.put("stateDetailed", city.getString("stateDetailed"));// 最低温度
//                    map.put("tem1", city.getString("tem1"));//天气
//                    map.put("tem2", city.getString("tem2"));// 发布时间
//                    map.put("windState", city.getString("windState"));// 发布时间
                      result=city.getString("quName")+","+city.getString("cityname")
                            +":"+city.getString("stateDetailed")+","+city.getString("tem1")
                            +"~"+city.getString("tem2")+","+city.getString("windState");
                }
            }
